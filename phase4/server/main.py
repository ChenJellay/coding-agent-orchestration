...
class HelixPaths:
    ...
    def repo_map_path(self) -> Path:
        # Phase 1 map file name isn't standardized yet in this repo.
        # We expose a single endpoint that returns the first known candidate if present.
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                # Ignore corrupted lines; logs are append-only.
                continue
...
def _feature_column_from_state(
    ...
    # If we don't have a DAG spec yet, treat as Scoping.
    if spec is None:
        return "SCOPING"

    # If we have no execution state, assume orchestration hasn't started.
    if state is None:
    ...
    # Hard-blocked / escalated surfaces as Blocked.
    if any(s in {"FAILED", "ESCALATED"} for s in statuses):
        return "BLOCKED"

    # If Phase 2 status is BLOCKED it appears in verification_status; also treat as Blocked.
    if any((isinstance(n, dict) and n.get("verification_status") == "BLOCKED") for n in nodes.values()):
        return "BLOCKED"

    # Verifying is derived from recent events.
    # (Phase3 doesn't have an explicit node status for verifying; judges run inside a node.)
    # If we see a judge-related message for this run recently, place in Verifying.
    judge_msgs = {
    ...
    # If all nodes passed, it's ready for review.
    if statuses and all(s == "PASSED_VERIFICATION" for s in statuses):
        return "READY_FOR_REVIEW"

    # Otherwise, orchestrating.
    return "ORCHESTRATING"


def _confidence_score(counts: Dict[str, int]) -> float:
    # Heuristic v1: ratio of passed nodes to total nodes, with penalty if any failed/blocked.
...
def _eta_seconds(counts: Dict[str, int]) -> Optional[int]:
    # Heuristic v1: assume 90s per remaining node.
    total = sum(counts.values())
    if total <= 0:
        return None
    passed = counts.get("PASSED_VERIFICATION", 0)
...
def _derive_features(limit: int = 200) -> List[Dict[str, Any]]:
    dag_ids = _list_dag_ids()
    dag_ids = dag_ids[:limit]

    # Load events once for derivations.
    ...
    # Sort by column, then most recently updated (best-effort via state mtime).
    col_order = {
        "SCOPING": 0,
        "ORCHESTRATING": 1,
        "BLOCKED": 2,
    ...
    features.sort(
        key=lambda f: (col_order.get(f["column"], 999), -_state_mtime(str(f["dag_id"]))),
    )
    return features
...
def _derive_triage(limit: int = 200) -> List[Dict[str, Any]]:
    ...
    # Enrich blocked items with a best-effort latest judge/rollback message.
    events = list(_iter_jsonl(PATHS.events_path))
...
def create_app() -> FastAPI:
    app = FastAPI(title="Agenti-Helix Phase 4 API", version="0.1.0")
    ...
    def list_dags() -> List[Dict[str, Any]]:
        ...
        for dag_id in _list_dag_ids():
            ...
            out.append(
                {
                    "dag_id": dag_id,
                    "macro_intent": spec.get("macro_intent"),
                    "mtime": mtime,
                }
            )
        return out
    ...
    def get_agents() -> Dict[str, Any]:
        # Heuristic v1: infer agents from event payloads when present; otherwise return empty.
        # (Current repo doesn't consistently emit agent identity yet.)
        return {"agents": []}

    @app.get("/api/compute")
    def get_compute() -> Dict[str, Any]:
        # Heuristic v1: use event rate as a proxy for compute burn.
        events = list(_iter_jsonl(PATHS.events_path))
        return {
    ...
    def get_repo_map() -> JSONResponse:
        ...
        for p in candidates:
            if p.exists():
                # Return raw text to support either JSON or JSONL.
                return JSONResponse(
                    content={
                        "path": str(p),
