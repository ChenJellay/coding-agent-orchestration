...
from phase2.checkpointing import EditTaskSpec
...
def _resolve_target_file(repo_root: Path, target_file: str) -> str:
    ...
    The LLM may guess wrong casing (Header.tsx vs header.js) or extension.
    We try:
    - Exact match
    - Case-insensitive path match within repo
    ...
    # Same dir + same stem (any extension).
    ...
    # Global stem match, prefer shortest then lexicographic.
    ...
    # Give up and keep original (Phase 2 will fail deterministically).
    return target_file


def _default_llm_compiler_url() -> str:
    # Mirror the Phase 2 judge default: local service on port 8000.
...
def compile_macro_intent_with_llm(
    ...
    *,
    ...
    """
    Call a local LLM service to break a macro-intent into a DAG of micro-tasks.

    Expected HTTP contract (POST JSON to /intent-compiler):
    Request body:
      {
        "macro_intent": "<full natural language intent>",
        "repo_path": "<absolute repo path>"
      }

    Response body:
      {
        "dag_id": "string (optional, overrides provided dag_id)",
        "nodes": [
          {
            "node_id": "N1",
            "description": "Update header color",
    ...
    try:
        ...
        log_event(
            run_id="intent",
            hypothesis_id="LLM",
            location="phase3/intent_compiler.py:compile_macro_intent_with_llm",
    ...
    try:
        ...
        raise RuntimeError(
            f"Invalid JSON from intent compiler: {exc}; payload={raw!r}"
        ) from exc
...
def compile_macro_intent_deterministic(
    ...
    *,
    ...
    """
    Deterministic fallback compiler used for the demo repo.

    This mirrors the earlier Phase 3 implementation: a 3-node linear DAG all
    targeting the header component.
    """
    repo_root = Path(repo_path).resolve()
...
def compile_macro_intent_to_dag(
    ...
    *,
    ...
    """
    Public entry point for compiling macro-intent into a DagSpec.

    - If use_llm is True (default), we call the local LLM-based intent compiler.
    - On transport/JSON errors, callers can decide whether to catch or let the
      exception surface; for the demo CLI we can fall back to deterministic.
    """
    if use_llm:
